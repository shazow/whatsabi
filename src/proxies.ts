...
export class GnosisSafeProxyResolver
	extends BaseProxyResolver
	implements ProxyResolver
{
	async resolve(provider: StorageProvider, address: string): Promise<string> {
		...
		return addressFromPadded(
			await provider.getStorageAt(address, slotPosition),
		);
	}
}
...
export class LegacyUpgradeableProxyResolver
	extends BaseProxyResolver
	implements ProxyResolver
{
	async resolve(provider: StorageProvider, address: string): Promise<string> {
...
export class EIP1967ProxyResolver
	extends BaseProxyResolver
	implements ProxyResolver
{
	async resolve(
		...
		const implAddr = addressFromPadded(
			await provider.getStorageAt(address, slots.EIP1967_IMPL),
		);
		...
		const fallbackAddr = addressFromPadded(
			await provider.getStorageAt(address, slots.EIP1967_BEACON),
		);
...
export class DiamondProxyResolver
	extends BaseProxyResolver
	implements ProxyResolver
{
	async resolve(
		...
		const facetMappingSlot = joinSlot([
			selector.padEnd(64, "0"),
			slots.DIAMOND_STORAGE,
		]);
	...
	// @throws {never} only throws unexpected errors
	async facets(
		...
		for (const f of facets) {
			...
			const selectors = await readArray(
				provider,
				address,
				facetSelectorsSlot,
				selectorWidth,
			);
			facetSelectors[addressWithChecksum(facet)] = selectors.map(
				(s) => "0x" + s,
			);
		}
...
export class ZeppelinOSProxyResolver
	extends BaseProxyResolver
	implements ProxyResolver
{
	async resolve(provider: StorageProvider, address: string): Promise<string> {
		return addressFromPadded(
			await provider.getStorageAt(address, slots.ZEPPELINOS_IMPL),
		);
	}
}

export class PROXIABLEProxyResolver
	extends BaseProxyResolver
	implements ProxyResolver
{
	async resolve(provider: StorageProvider, address: string): Promise<string> {
		return addressFromPadded(
			await provider.getStorageAt(address, slots.PROXIABLE),
		);
	}
}
...
export class SequenceWalletProxyResolver
	extends BaseProxyResolver
	implements ProxyResolver
{
	async resolve(provider: StorageProvider, address: string): Promise<string> {
		return addressFromPadded(
			await provider.getStorageAt(address, address.toLowerCase().slice(2)),
		);
	}
...
// Example: EIP-1167
export class FixedProxyResolver
	extends BaseProxyResolver
	implements ProxyResolver
{
...
export const slotResolvers: Record<string, ProxyResolver> = {
	...
	["0xc8fcad8db84d3cc18b4c41d551ea0ee66dd599cde068d998e57d5e09332c131d"]:
		new DiamondProxyResolver("DiamondProxy"),
};
