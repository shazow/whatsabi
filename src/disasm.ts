...
// Disassembles bytecode into an ABI
// @throws {never} only would throw if the bytecode is invalid
export function abiFromBytecode(bytecodeOrProgram: string | Program): ABI {
	const p =
		typeof bytecodeOrProgram === "string"
			? disasm(bytecodeOrProgram)
			: bytecodeOrProgram;
	...
	for (const [selector, offset] of Object.entries(p.selectors)) {
		...
		if (
			tags.has(opcodes.CALLDATALOAD) ||
			tags.has(opcodes.CALLDATASIZE) ||
			tags.has(opcodes.CALLDATACOPY)
		) {
...
export function disasm(
	...
	while (code.hasMore()) {
		...
		if (inst === opcodes.JUMPDEST) {
			...
			if (isHalt(code.at(-2)) || code.at(-2) === opcodes.JUMP) {
				...
				if (resumeJumpTable.delete(pos)) {
					...
					checkJumpTable =
						code.at(pos + 1) === opcodes.DUP1 ||
						code.at(pos + 1) === opcodes.CALLDATALOAD;
				} else if (
					!checkJumpTable &&
					resumeJumpTable.size === 0 &&
					onlyJumpTable
				) {
		...
		if (currentFunction.opTags !== undefined) {
			...
			if (
				(inst === opcodes.JUMP || inst === opcodes.JUMPI) &&
				isPush(code.at(-2))
			) {
		...
		//   ... PUSHN <BYTEN> JUMPI
		if (!(code.at(-1) === opcodes.JUMPI && isPush(code.at(-2)))) continue;
		...
		//                                        ^ We are here (at -1)
		if (code.at(-3) === opcodes.SUB && isPush(code.at(-4))) {
...
function subtreeTags(
	entryFunc: Function,
	dests: { [key: number]: Function },
): Set<OpCode> {
